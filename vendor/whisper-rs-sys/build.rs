...
fn main() {
    ...
    // Tarjama fork: forward TARJAMA_GGML_<NAME> env vars as real CMake cache
    // variables named GGML_<NAME>. The stock crate forwards WHISPER_* verbatim,
    // which whisper.cpp's CMakeLists never reads for SIMD control, so env flags
    // like WHISPER_NO_AVX2 were silently ignored and the build machine's CPU
    // features (/arch:AVX2 via FindSIMD under GGML_NATIVE=ON) got baked into
    // the binary - crashing with illegal-instruction on CPUs lacking them.
    // Explicit cache vars override ggml's option() defaults, e.g.:
    //   TARJAMA_GGML_NATIVE=OFF TARJAMA_GGML_AVX2=OFF ...
    for (key, value) in env::vars() {
        if let Some(name) = key.strip_prefix("TARJAMA_GGML_") {
            let cmake_name = format!("GGML_{name}");
            println!("cargo:warning=ggml option override: {cmake_name}={value}");
            config.define(&cmake_name, &value);
        }
    }
